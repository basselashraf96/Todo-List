...
import { dbFindUserWithObjection } from "@lib/db/users";
import { JwtPayload } from "@lib/util/types";
import jwt from "jsonwebtoken";
import { UserModel } from "@lib/db/postgres/models/user";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const user: UserModel | null = await dbFindUserWithObjection(
      username,
      password
    );
